...
let GuestCartsService = class GuestCartsService {
    ...
    async addItem(dto) {
        const cart = await this.guestCartModel.findOneAndUpdate({ sessionId: dto.sessionId }, { $setOnInsert: { sessionId: dto.sessionId, items: [] } }, { upsert: true, returnDocument: 'after' });
    ...
    async clearCart(sessionId) {
        const cart = await this.guestCartModel.findOneAndUpdate({ sessionId }, { items: [] }, { returnDocument: 'after', upsert: true });
        return cart;
    }
};
