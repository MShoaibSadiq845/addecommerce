...
let CartsService = class CartsService {
    ...
    async addItem(userId, dto) {
        const cart = await this.cartModel.findOneAndUpdate({ user: userId }, { $setOnInsert: { user: new mongoose_2.Types.ObjectId(userId) } }, { upsert: true, returnDocument: 'after' });
    ...
    async updateItem(userId, itemId, dto) {
        ...
        const item = cart.items.find((i) => i._id?.toString() === itemId || i.id === itemId || i.product?.toString() === itemId);
        if (!item)
            return cart;
    ...
    async clearCart(userId) {
        const cart = await this.cartModel.findOneAndUpdate({ user: userId }, { items: [] }, { returnDocument: 'after', upsert: true }).exec();
        return cart;
    }
};
