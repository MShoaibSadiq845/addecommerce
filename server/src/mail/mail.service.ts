...
export class MailService {
  ...
  /**
   * Sends a professional HTML order confirmation email to the specified user email address using Resend.
   *
   * @param userEmail - Recipient email address
   * @param orderDetails - Order object or details containing order items, shipping address, payment method, etc.
   */
  async sendOrderConfirmationEmail(userEmail: string, orderDetails: any): Promise<any> {
    if (!userEmail) {
      throw new InternalServerErrorException('Recipient email address is required');
    }
    ...
    // Format items breakdown HTML & text
    ...
    try {
      ...
      const fromAddress = this.configService?.get<string>('RESEND_FROM_EMAIL') || 'Fab Decor Co <support@fabdecorco.com>';
      ...
      if (error) {
        ...
        throw new InternalServerErrorException(
          `Failed to send order confirmation email: ${error.message || JSON.stringify(error)}`,
        );
      }
      ...
      throw new InternalServerErrorException(
        err?.message || 'Internal server error while sending order confirmation email via Resend',
      );
    }
  }
}
