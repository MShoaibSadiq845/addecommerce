              ...
              {orders.map((order: any) => {
                ...
                return (
                  <tr key={order._id} className="hover:bg-gray-50 transition-all">
                    <td className="px-5 py-4 font-bold text-black font-mono">#{order._id.slice(-6)}</td>
                    <td className="px-5 py-4">
                      <div className="flex flex-col">
                        <span className="font-bold text-gray-900">{order.guestName || 'Guest'}</span>
                        <span className="text-[10px] text-gray-400">{order.guestEmail}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4 text-gray-600">{new Date(order.createdAt).toLocaleDateString()}</td>
                    <td className="px-5 py-4 font-extrabold text-black">₨{order.totalAmount?.toLocaleString()}</td>
                    <td className="px-5 py-4 text-gray-600">{order.items?.length || 0} item(s)</td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <select value={order.status} onChange={(e) => handleStatusChange(order._id, e.target.value)}
                          disabled={isThisUpdating}
                          className={`text-[10px] font-bold px-2.5 py-1.5 rounded-full border outline-none disabled:opacity-50 ${STATUS_STYLES[order.status] || 'bg-gray-100 text-gray-700'}`}>
                          {['Pending','Processing','Shipped','Delivered','Canceled'].map((s) => (
                            <option key={s} value={s}>{s}</option>
                          ))}
                        </select>
                        {isThisUpdating && <Loader2 className="w-3.5 h-3.5 animate-spin text-gray-400 shrink-0" />}
                      </div>
                    </td>
