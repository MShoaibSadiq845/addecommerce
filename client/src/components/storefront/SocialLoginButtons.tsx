...
export function SocialLoginButtons({ labelPrefix = 'Sign in with', compact = false }: SocialLoginButtonsProps) {
  ...
  return (
    <div className="flex flex-col gap-2.5 w-full">
      ...
      <div className={`grid ${compact ? 'grid-cols-3 gap-2' : 'grid-cols-1 sm:grid-cols-3 gap-2.5'} mt-1.5`}>
        {/* Google Button */}
        <button
          type="button"
          onClick={() => handleSocialLogin('google')}
          className={`flex items-center justify-center gap-1.5 ${
            compact ? 'py-2 px-2 text-xs rounded-xl' : 'py-3 px-3 text-sm rounded-2xl'
          } bg-white border border-gray-200 hover:border-black hover:bg-gray-50 transition-all shadow-sm font-bold text-gray-700 active:scale-95`}
          title="Sign in with Google"
        >
          <svg className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} shrink-0`} viewBox="0 0 24 24">
            <path
              fill="#4285F4"
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 ${
            compact ? 'py-2 px-2 text-xs rounded-xl' : 'py-3 px-3 text-sm rounded-2xl'
          } bg-zinc-900 hover:bg-black text-white transition-all shadow-sm font-bold active:scale-95`}
          title="Sign in with GitHub"
        >
          <svg className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} fill-current shrink-0`} viewBox="0 0 24 24">
        ...
        <button
          ...
          className={`flex items-center justify-center gap-1.5 ${
            compact ? 'py-2 px-2 text-xs rounded-xl' : 'py-3 px-3 text-sm rounded-2xl'
          } bg-[#5865F2] hover:bg-[#4752C4] text-white transition-all shadow-sm font-bold active:scale-95`}
          title="Sign in with Discord"
        >
          <svg className={`${compact ? 'w-3.5 h-3.5' : 'w-4 h-4'} fill-current shrink-0`} viewBox="0 0 127.14 96.36">
