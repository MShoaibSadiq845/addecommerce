...
export default function CanceledOrders() {
  ...
  // Query server database specifically for status: 'Canceled' and search term
  const {
    data: orders = [],
    isLoading,
    isFetching,
    ...
    { status: 'Canceled', search: debouncedSearch },
  ...
  // Stats calculations from returned canceled orders
  const totalCanceledCount = orders.length;
  const totalCanceledValue = orders.reduce((sum: number, o: any) => sum + (o.totalAmount || 0), 0);
  ...
  return (
    <div className="flex flex-col gap-6 font-['Rubik']">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-gray-900">Canceled Orders</h1>
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 border border-red-200">
                <AlertTriangle className="w-3.5 h-3.5" /> Live DB Sync
              </span>
            </div>
            <p className="text-xs text-gray-500 mt-0.5">
              All canceled customer orders with real-time server database search
            </p>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-r from-red-950 via-gray-900 to-black p-6 rounded-2xl shadow-md text-white flex flex-col gap-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 text-xs font-bold text-red-400">
            <Database className="w-4 h-4 animate-pulse" />
            <span>MongoDB Server-Side Database Search (Canceled)</span>
          </div>
          {isFetching && (
            <span className="inline-flex items-center gap-1 text-[11px] text-gray-300 font-medium">
              <Loader2 className="w-3 h-3 animate-spin text-red-400" /> Querying MongoDB Database...
            </span>
          )}
        </div>
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Search canceled orders by Customer Name, Email, Phone, Order ID, City, or Item..."
            className="w-full pl-12 pr-12 py-3.5 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl text-sm text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all"
          />
          {searchInput && (
            <button
        ...
        <div className="flex items-center justify-between text-xs text-gray-400 px-1">
          <span>
            {debouncedSearch ? (
              <>Filtering canceled orders matching <strong className="text-white">"{debouncedSearch}"</strong></>
            ) : (
              'Type to query MongoDB directly across names, emails, phones, IDs & addresses.'
            )}
          </span>
          <span className="font-semibold text-gray-300">
            Results found: <span className="text-red-400 font-bold">{totalCanceledCount}</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Canceled Orders</p>
            <h3 className="text-2xl font-bold text-gray-900 mt-0.5">{totalCanceledCount}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Canceled Value</p>
            <h3 className="text-2xl font-bold text-gray-900 mt-0.5">₨{totalCanceledValue.toLocaleString()}</h3>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 shadow-sm flex flex-col items-center justify-center gap-3">
          ...
          <h3 className="text-base font-bold text-gray-800">No Canceled Orders Found</h3>
          <p className="text-xs text-gray-500 max-w-sm">
            {debouncedSearch
              ? `No canceled orders in MongoDB match your search "${debouncedSearch}".`
              : 'There are currently no orders with Canceled status.'}
          </p>
          {debouncedSearch && (
            <button
              onClick={() => setSearchInput('')}
          ...
          <table className="w-full text-left text-xs min-w-[780px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {paginatedOrders.map((order: any) => {
                ...
                return (
                  <tr key={order._id} className="hover:bg-red-50/40 transition-all">
